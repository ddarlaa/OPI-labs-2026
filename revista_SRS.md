...
```mermaid
Судя по скриншоту, ошибка возникает из-за синтаксиса стрелок (в частности наследования `-.-|>`), который некоторые версии рендереров (например, на GitHub) в режиме `graph` не понимают, либо из-за отсутствия пробелов/переносов строк.

Чтобы диаграмма гарантированно заработала везде, мы упростим стрелки наследования до обычных пунктирных (`-.->`) и используем самый надежный синтаксис.

Вот исправленный и адаптированный под вашу логику код. Скопируйте его целиком:
